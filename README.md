...
| `edge: no audio after 5 attempts` | Microsoft's endpoint drops roughly one call in twelve, in bursts of one or two. Text length, voice, content and idle time were each measured and ruled out — there is no input to avoid, only a window to outlast, which is what the ~15s retry budget is for. Seeing this means all five attempts plus the cross-line retry failed, so check the network. |
