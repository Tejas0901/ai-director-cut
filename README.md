...
| `edge: No audio was received` | Microsoft's endpoint drops roughly one call in twelve, transiently — the same text works on the next try. Narration already retries three times, so seeing this means all three failed; check the network before the voice name. |
