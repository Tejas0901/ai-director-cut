...
def synthesize(text: str, out_path: str | Path, provider: str | None = None) -> Path:
    ...
    text = (text or "").strip()
    if not text:
        return _silence(out_path, 0.4)

    try:
        if provider == "piper":
            return _piper(text, out_path)
        if provider == "mock":
            return _silence(out_path, _estimate_seconds(text))
...
if __name__ == "__main__":
    ...
    print(f"wrote {result}  ({duration_of(result):.2f}s)")
